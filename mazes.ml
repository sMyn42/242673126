...
module MakeMazePuzzleDescription (M : MAZEINFO)
  ...
  struct
    ...
    let execute_moves (path : move list) : state =
     ...
     (* let right_of (dir : direction) : direction =
        match dir with
        | Up -> Right
        | Right -> Down
        | Down -> Left
    ...
    let draw_maze (maze_map : space array array)
      ...
      Array.iteri (fun y m -> 
                   Array.iteri (fun x n -> 
                                match n with
                                | EmptySpace -> draw_square cUNSEENCOLOR 
                                                            y 
                                                            x 
                                                            elt_width 
                                                            elt_height
                                | Wall -> draw_square cWALLCOLOR 
                                                      y 
                                                      x 
                                                      elt_width 
                                                      elt_height
    ...
    let display_maze (dims : int * int)
      ...
      List.iter (fun (y, x) ->
                 ...
                 draw_square cGOALBGCOLOR 
                             (fst goal) 
                             (snd goal) 
                             elt_width 
                             elt_height;
                 draw_circle cGOALCOLOR 
                             (fst goal) 
                             (snd goal) 
                             elt_width 
                             elt_height;
    ...
    let draw (visited : state list) (path : move list) : unit =
      ...
      display_maze M.dims 
                   M.maze 
                   visited 
                   (moves_to_states initial_state path) 
                   goal_state
                   (*let right_of (dir : direction) : direction =
                    match dir with
                    | Up -> Right
                  ...
                  let left_of (dir : direction) : direction =
                    match dir with
                    | Up -> Left
                    | Left -> Down
                    | Down -> Right
                  ...
                  let opposite_of (dir : direction) : direction =
                    match dir with
                    | Up -> Down
                    | Down -> Up
                    | Left -> Right
                 ...
                 (* let current_direction (_ : state) : direction = Right*)
                                end
